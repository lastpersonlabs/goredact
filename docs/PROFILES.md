...
removed, update this file by hand in the same change.

Drift is a test failure, not a documentation nit:
`TestProfilesDocMatchesBuiltins` (`internal/rules/profilesdoc_test.go`)
compares every row below, the total rule count, and the per-profile counts
against the compiled `Builtins()` table, and reports the exact stale rows.
A quick way to regenerate the table body:
