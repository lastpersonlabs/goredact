...
func TestNotionInternalToken(t *testing.T) {
	...
	cases := []paymentsCase{
		...
		{name: "ntn_ preceded by identifier byte rejected (embedded occurrence)", window: "client_" + ntnMatch46, trigStart: 7, trigEnd: 11, wantOK: false},
		{name: "ntn_ preceded by non-identifier byte accepted", window: "=" + ntnMatch46, trigStart: 1, trigEnd: 5, wantOK: true, wantEnd: 1 + len(ntnMatch46)},
